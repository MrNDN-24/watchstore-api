...
const callBackPayment = async (req, res) => {
  try {
    ...
    // 2. Tìm thanh toán trong database với điều kiện isDelete: false
    const payment = await Payment.findOne({ _id: orderId, isDelete: false }); // Thêm điều kiện isDelete: false
    if (!payment) {
      return res
        .status(404)
        .json({ success: false, message: "Payment not found or deleted." });
    }
    ...
    if (!order) {
      return res
        .status(404)
        .json({ success: false, message: "Order not found or deleted." });
    }
